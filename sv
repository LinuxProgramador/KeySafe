...
from cryptography.hazmat.primitives.constant_time import memzero
...
class SecureVault:
    ...
    def generate_key(self):
       ...
       try:
        ...
        return generated_key
         ...
         memzero(generated_key)
         memzero(char)
         memzero(key_length)
         memzero(query_longitude)
    ...
    def auxiliary_save_key(self,key_name,temp_entry,temp_encrypt,temp_fernet_key):
      ...
      with open(path.join(self.key_path,key_name), 'wb') as key_file:
           try:
            ...
            memzero(temp_encrypt)
            memzero(temp_fernet_key)
            memzero(fernet)
    ...
    def auxiliary_change_unique_key(self,key,fernet_old_key,new_fernet_key):
      ...
      memzero(fernet_old_key)
      memzero(new_fernet_key)
      memzero(decrypted_content)
      memzero(fernet_new_key)
      return
